...
`adversarial-review --background` detaches its own worker and returns IMMEDIATELY with a job id. Run it in the **FOREGROUND**. Do NOT wrap it in `Bash(..., run_in_background: true)`:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" adversarial-review --background "$ARGUMENTS"
```
...
- This command returns **structured JSON** (see `prompts/adversarial-review.md`), so a truncated or empty payload is a collection failure, not an `approve` verdict. Never read missing output as "no findings."
